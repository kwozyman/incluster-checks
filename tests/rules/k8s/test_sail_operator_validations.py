"""Tests for Sail Operator installation validations."""
...
_ALL_CRDS = [crd for crd, _ in VerifySailOperatorInstalled.REQUIRED_CRDS]
...
class TestVerifySailOperatorInstalled(RuleTestBase):
    """Test VerifySailOperatorInstalled rule."""

    tested_type = VerifySailOperatorInstalled

    scenario_passed = [
        RuleScenarioParams(
            "all required Sail Operator CRDs are present",
    ...
    scenario_failed = [
        RuleScenarioParams(
            "all required Sail Operator CRDs are missing",
            tested_object_mock_dict={
                "oc_api.crd_exists": Mock(side_effect=_crd_exists_side_effect(set(_ALL_CRDS))),
            },
            failed_msg=(
                "Sail Operator is not fully installed - missing CRDs: "
                + ", ".join(_ALL_CRDS)
            ),
        ),
        RuleScenarioParams(
            "some Sail Operator CRDs are missing",
            tested_object_mock_dict={
                "oc_api.crd_exists": Mock(
                    side_effect=_crd_exists_side_effect(
                        {"istios.sailoperator.io", "ztunnels.sailoperator.io"}
                    )
                ),
            },
            failed_msg=(
    ...
    def test_scenario_passed(self, scenario_params, tested_object):
        """Test that rule passes when all Sail Operator CRDs exist."""
        RuleTestBase.test_scenario_passed(self, scenario_params, tested_object)

    @pytest.mark.parametrize("scenario_params", scenario_failed)
    def test_scenario_failed(self, scenario_params, tested_object):
        """Test that rule fails when Sail Operator CRDs are missing."""
    ...
    def test_checks_each_required_crd_with_correct_scope(self, tested_object):
        ...
        assert result
        assert mock_crd_exists.call_count == len(VerifySailOperatorInstalled.REQUIRED_CRDS)
        for crd_name, cluster_wide in VerifySailOperatorInstalled.REQUIRED_CRDS:
