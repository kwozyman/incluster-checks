"""Tests for KServe installation validations."""
...
_ALL_CRDS = [crd for crd, _ in VerifyKServeInstalled.REQUIRED_CRDS]
...
class TestVerifyKServeInstalled(RuleTestBase):
    """Test VerifyKServeInstalled rule."""

    tested_type = VerifyKServeInstalled

    scenario_passed = [
        RuleScenarioParams(
            "all required KServe CRDs are present",
    ...
    scenario_failed = [
        RuleScenarioParams(
            "all required KServe CRDs are missing",
            tested_object_mock_dict={
                "oc_api.crd_exists": Mock(side_effect=_crd_exists_side_effect(set(_ALL_CRDS))),
            },
            failed_msg=(
                "KServe is not fully installed - missing CRDs: " + ", ".join(_ALL_CRDS)
            ),
        ),
        RuleScenarioParams(
            "some KServe CRDs are missing",
            tested_object_mock_dict={
            ...
            failed_msg=(
                "KServe is not fully installed - missing CRDs: "
                "llminferenceservices.serving.kserve.io, "
                "inferencepools.inference.networking.x-k8s.io"
            ),
        ),
    ...
    def test_scenario_passed(self, scenario_params, tested_object):
        """Test that rule passes when all KServe CRDs exist."""
        RuleTestBase.test_scenario_passed(self, scenario_params, tested_object)

    @pytest.mark.parametrize("scenario_params", scenario_failed)
    def test_scenario_failed(self, scenario_params, tested_object):
        """Test that rule fails when KServe CRDs are missing."""
    ...
    def test_checks_each_required_crd_with_correct_scope(self, tested_object):
        ...
        assert result
        assert mock_crd_exists.call_count == len(VerifyKServeInstalled.REQUIRED_CRDS)
        for crd_name, cluster_wide in VerifyKServeInstalled.REQUIRED_CRDS:
