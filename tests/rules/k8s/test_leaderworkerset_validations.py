"""Tests for LeaderWorkerSet installation validations."""
...
_ALL_CRDS = [crd for crd, _ in VerifyLeaderWorkerSetInstalled.REQUIRED_CRDS]
...
class TestVerifyLeaderWorkerSetInstalled(RuleTestBase):
    """Test VerifyLeaderWorkerSetInstalled rule."""

    tested_type = VerifyLeaderWorkerSetInstalled

    scenario_passed = [
        RuleScenarioParams(
            "required LeaderWorkerSet CRD is present",
    ...
    scenario_failed = [
        RuleScenarioParams(
            "required LeaderWorkerSet CRD is missing",
            tested_object_mock_dict={
                "oc_api.crd_exists": Mock(side_effect=_crd_exists_side_effect(set(_ALL_CRDS))),
            },
            failed_msg=(
                "LeaderWorkerSet is not fully installed - missing CRDs: " + ", ".join(_ALL_CRDS)
            ),
        ),
    ]
    ...
    def test_scenario_passed(self, scenario_params, tested_object):
        """Test that rule passes when the LeaderWorkerSet CRD exists."""
        RuleTestBase.test_scenario_passed(self, scenario_params, tested_object)

    @pytest.mark.parametrize("scenario_params", scenario_failed)
    def test_scenario_failed(self, scenario_params, tested_object):
        """Test that rule fails when the LeaderWorkerSet CRD is missing."""
    ...
    def test_checks_each_required_crd_with_correct_scope(self, tested_object):
        ...
        assert result
        assert mock_crd_exists.call_count == len(VerifyLeaderWorkerSetInstalled.REQUIRED_CRDS)
        for crd_name, cluster_wide in VerifyLeaderWorkerSetInstalled.REQUIRED_CRDS:
