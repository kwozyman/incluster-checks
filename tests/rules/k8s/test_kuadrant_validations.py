"""Tests for Kuadrant installation validations."""
...
_ALL_CRDS = [crd for crd, _ in VerifyKuadrantInstalled.REQUIRED_CRDS]
...
class TestVerifyKuadrantInstalled(RuleTestBase):
    """Test VerifyKuadrantInstalled rule."""

    tested_type = VerifyKuadrantInstalled

    scenario_passed = [
        RuleScenarioParams(
            "all required Kuadrant CRDs are present",
    ...
    scenario_warning = [
        RuleScenarioParams(
            "all required Kuadrant CRDs are missing",
            tested_object_mock_dict={
                "oc_api.crd_exists": Mock(side_effect=_crd_exists_side_effect(set(_ALL_CRDS))),
            },
            failed_msg=(
                "Kuadrant is not fully installed - missing CRDs: " + ", ".join(_ALL_CRDS)
            ),
        ),
        RuleScenarioParams(
            "some Kuadrant CRDs are missing",
            tested_object_mock_dict={
            ...
            failed_msg=(
                "Kuadrant is not fully installed - missing CRDs: "
                "authpolicies.kuadrant.io, limitadors.limitador.kuadrant.io"
            ),
        ),
    ]
    ...
    def test_scenario_passed(self, scenario_params, tested_object):
        """Test that rule passes when all Kuadrant CRDs exist."""
    ...
    def test_scenario_warning(self, scenario_params, tested_object):
        """Test that rule warns when Kuadrant CRDs are missing."""
    ...
    def test_checks_each_required_crd_with_correct_scope(self, tested_object):
        ...
        assert result
        assert mock_crd_exists.call_count == len(VerifyKuadrantInstalled.REQUIRED_CRDS)
        for crd_name, cluster_wide in VerifyKuadrantInstalled.REQUIRED_CRDS:
